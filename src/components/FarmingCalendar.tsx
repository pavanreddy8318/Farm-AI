...
import { 
  ...
  ChevronRight, RefreshCw, AlertCircle, Trash, Check, Info, Droplets, Leaf, Clock
} from 'lucide-react';
...
export default function FarmingCalendar() {
  const { scheduleReminder, cancelReminder, reminders } = useNotifications();
  ...
  // Field observation notes (persisted in localStorage)
  const [observationNotes, setObservationNotes] = useState<Record<string, string>>(() => {
    try {
      const saved = localStorage.getItem('farmai_observation_notes');
  ...
  const handleUpdateNote = (taskId: string, text: string) => {
    setObservationNotes((prev) => {
      const updated = { ...prev, [taskId]: text };
      localStorage.setItem('farmai_observation_notes', JSON.stringify(updated));
      return updated;
    });
  };
  ...
  const fetchHistory = async () => {
    try {
      const response = await fetch('/api/generate-plan/history');
      ...
      console.warn('Failed to load farming plans history:', err);
    }
  };
  ...
  const handleGeneratePlan = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setPlan(data);
      fetchHistory(); // Refresh dynamic history list
    } catch (err: any) {
  ...
  return (
    <div id="calendar-module" className="space-y-8">
                    ...
                    return (
                      <div key={task.id} id={`timeline-step-${task.id}`} className="relative space-y-2 group">
                        ...
                        <div className={`p-5 bg-white border rounded-none shadow-xs transition-all ${
                          ...
                          <div className="mt-4 pt-3.5 border-t border-editorial-border/40 space-y-2">
                            <div className="flex items-center justify-between">
                              <span className="text-[8px] font-bold uppercase tracking-widest text-editorial-sage font-sans">
                                Field Notes & Observations
                              </span>
                              <span className="text-[8px] font-mono text-neutral-400">
                                Dictate crop observations
                              </span>
                            </div>
                            <div className="flex gap-2">
                              <textarea
                                value={observationNotes[task.id] || ''}
                                onChange={(e) => handleUpdateNote(task.id, e.target.value)}
                                placeholder="Tap mic to dictate field observations, leaf discoloration, moisture levels, etc..."
                                className="flex-1 text-xs px-3 py-2 border border-editorial-border rounded-none focus:outline-none focus:border-editorial-dark bg-editorial-sand/15 text-editorial-dark font-serif h-14 resize-none placeholder:text-neutral-400"
                              />
                              <VoiceDictator 
                                onTranscript={(transcript) => {
                                  const current = observationNotes[task.id] || '';
                                  handleUpdateNote(task.id, current ? `${current} ${transcript}` : transcript);
                                }}
                                size="sm"
                                className="h-14"
                              />
                          ...
                          <div className="mt-4 pt-3.5 border-t border-editorial-border/40 flex flex-wrap items-center justify-between gap-3 bg-neutral-50 p-2.5">
                            <div className="flex items-center gap-1.5">
                              <Clock className="w-4 h-4 text-editorial-sage" />
                              <span className="text-[9px] font-sans font-bold uppercase tracking-wider text-editorial-sage">
                            ...
                            {(() => {
                              const activeRem = reminders.find(
                                (r) => r.cropName === plan.cropName && r.taskTitle === task.taskTitle
                              );

                              if (activeRem) {
                                return (
                                  <div className="flex items-center gap-2">
                                    <span className="text-[8px] bg-amber-50 text-amber-800 border border-amber-200 px-2 py-0.5 font-sans font-bold uppercase tracking-wider flex items-center gap-1">
                                      <span className="h-1.5 w-1.5 rounded-full bg-amber-500 animate-pulse"></span>
                                      Alarm Active
                                    </span>
                                    <button
                                      onClick={() => cancelReminder(activeRem.id)}
                              ...
                              const alertType: 'watering' | 'fertilizer' | 'maintenance' = task.fertilizerInfo 
                                ? 'fertilizer' 
                                : (task.taskTitle.toLowerCase().includes('water') || task.wateringFrequency.toLowerCase().includes('water')) 
                                  ? 'watering' 
                                  : 'maintenance';

                              return (
                                <div className="flex items-center gap-1">
                                  <span className="text-[8px] font-serif text-neutral-400 mr-1.5">Schedule for:</span>
                                  <button
                                    onClick={() => scheduleReminder(plan.cropName, task.taskTitle, alertType, task.stageName, 10)}
                                    className="px-2 py-0.5 bg-white hover:bg-editorial-sand text-[8px] border border-editorial-border font-sans font-bold uppercase tracking-wider text-editorial-dark transition-all"
                                    title="Watering alarm in 10s"
                                  >
                                    10s Test
                                  </button>
                                  <button
                                    onClick={() => scheduleReminder(plan.cropName, task.taskTitle, alertType, task.stageName, 60)}
                                    className="px-2 py-0.5 bg-white hover:bg-editorial-sand text-[8px] border border-editorial-border font-sans font-bold uppercase tracking-wider text-editorial-dark transition-all"
                                    title="Nutrition reminder in 1m"
                                  >
                                    1m
                                  </button>
                                  <button
                                    onClick={() => scheduleReminder(plan.cropName, task.taskTitle, alertType, task.stageName, 300)}
                                    className="px-2 py-0.5 bg-white hover:bg-editorial-sand text-[8px] border border-editorial-border font-sans font-bold uppercase tracking-wider text-editorial-dark transition-all"
                                    title="Maintenance checklist alert in 5m"
                                  >
                                    5m
                                  </button>
                                </div>
      ...
      {/* Historic Saved Schedules Section */}
      {history.length > 0 && (
        <div id="calendar-history" className="bg-white border border-editorial-border p-6 space-y-4">
          <div className="border-b border-editorial-border pb-3 flex items-center justify-between">
            <div>
              <h3 className="font-serif font-bold text-editorial-dark text-base flex items-center gap-2">
                <Calendar className="w-5 h-5 text-editorial-sage" /> Saved Planting Schedules
              </h3>
              <p className="text-[10px] text-editorial-sage font-serif">
                Access your previously generated crop cultivation timelines and phase directives saved in your cloud database profile.
              </p>
            </div>
            <span className="text-[10px] bg-editorial-sand px-2.5 py-1 text-editorial-dark font-sans font-bold uppercase tracking-wider">
            ...
            {history.map((record: any, idx) => (
              <div 
                ...
                onClick={() => {
                  ...
                  setSoilType(record.soilRequirements);
                  setRegion(record.climateRequirements);
                  setCompletedTasks({});
                  // Scroll to plan view
                  document.getElementById('calendar-checklist')?.scrollIntoView({ behavior: 'smooth' });
                }}
                className="group cursor-pointer border border-editorial-border hover:border-editorial-sage hover:bg-neutral-50/50 p-4 transition-all space-y-2"
              >
                <div className="flex items-center justify-between">
                  <span className="text-[9px] font-sans font-bold uppercase tracking-widest text-editorial-sage">
                    {record.variety || 'Standard Variety'}
                  </span>
                  <span className="text-[8px] font-mono text-neutral-400">
                ...
                <div>
                  <h4 className="font-serif font-bold text-sm text-editorial-dark group-hover:text-editorial-forest transition-colors">
                    {record.cropName} Cultivation Schedule
                  </h4>
                  <p className="text-[10px] text-neutral-500 font-serif line-clamp-1">
                    {record.soilRequirements} • {record.climateRequirements}
                  </p>
                </div>
                <div className="flex items-center justify-between pt-1 border-t border-editorial-border/40 text-[9px]">
                  <span className="text-neutral-400 font-mono">
                    {record.totalDurationDays} Days Lifecycle
                  </span>
                  <span className="text-editorial-forest font-sans font-bold uppercase tracking-wider group-hover:underline">
                    Load Calendar →
                  </span>
                </div>
              </div>
