...
import { 
  ...
  Bug, Eye, HeartHandshake, Leaf, Sprout, Layers
} from 'lucide-react';
import { DiseaseDiagnosis, DiseaseDetectionPipelineResult } from '../types';
...
export default function DiseaseDiagnostics() {
  ...
  // Fetch diagnostics history on mount
  ...
  const runDiagnosis = async (base64Clean: string, mime: string, notes: string, overrideMetrics?: { soilMoisturePct: number; soilPh: number; temperatureC: number; humidityPct: number }) => {
    try {
      // 1. Fetch standard pathology report
      ...
      // 2. Fetch automated YOLOv8 + PyTorch Geometric FGCN + Action Engine pipeline
      ...
      if (pipelineRes.ok) {
        const pData: DiseaseDetectionPipelineResult = await pipelineRes.json();
      ...
      setError(err.message || 'Diagnosis failed. Please check network connection.');
    } finally {
  ...
  const handleFieldMetricsChange = async (metrics: { soilMoisturePct: number; soilPh: number; temperatureC: number; humidityPct: number }) => {
    ...
    try {
      const base64Clean = selectedImage.includes(',') 
        ...
        : selectedImage;
      await runDiagnosis(base64Clean, mimeType, additionalNotes, metrics);
    } finally {
  ...
  return (
    <div id="diagnostics-module" className="space-y-8">
      ...
      <div id="main-diagnostics-grid" className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div id="diagnostics-results" className="lg:col-span-7 space-y-6">
          
          {/* Render Full Automated Pipeline Result (YOLOv8 + FGCN + Action Engine) */}
          {pipelineResult && (
            <div id="pipeline-result-wrapper" className="space-y-4">
              <DiseasePipelineCard
          ...
          {!diagnosis && !pipelineResult && !isAnalyzing && (
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="flex flex-col items-end">
                      ...
                      <span className="text-sm font-bold font-mono text-editorial-dark">{Math.round((diagnosis.confidenceScore ?? 0) * 100)}%</span>
                    </div>
      ...
      {/* Historic Scans & Diagnostic Library */}
      {history.length > 0 && (
        <div id="diagnostics-history" className="bg-white border border-editorial-border p-6 space-y-4">
          <div className="border-b border-editorial-border pb-3 flex items-center justify-between">
            <div>
              <h3 className="font-serif font-bold text-editorial-dark text-base flex items-center gap-2">
                <Activity className="w-5 h-5 text-editorial-sage" /> Saved Pathology Scan History
              </h3>
              <p className="text-[10px] text-editorial-sage font-serif">
                Access your previously uploaded foliage diagnostic records saved in your cloud database profile.
              </p>
            </div>
            ...
            {history.map((record: any) => (
              <div 
                ...
                onClick={() => {
                  ...
                  // Scroll to diagnostic details
                  document.getElementById('report-panel')?.scrollIntoView({ behavior: 'smooth' });
                }}
                className="group cursor-pointer border border-editorial-border hover:border-editorial-sage hover:bg-neutral-50/50 p-3 flex gap-3 transition-all items-center"
              >
                {record.imageUrl ? (
                  <img 
                    src={record.imageUrl} 
                    alt={record.cropName} 
                    referrerPolicy="no-referrer"
                    className="w-14 h-14 object-cover border border-editorial-border flex-shrink-0"
                  />
                ) : (
                  <div className="w-14 h-14 bg-editorial-sand/40 border border-editorial-border flex items-center justify-center flex-shrink-0 text-editorial-sage">
                    <Leaf className="w-6 h-6" />
                  </div>
                )}
                <div className="min-w-0 flex-1 space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-sans font-bold uppercase tracking-wider text-editorial-sage truncate">
                      {record.cropName}
                    </span>
                  ...
                  <h4 className="font-serif font-bold text-xs text-editorial-dark group-hover:text-editorial-forest truncate">
                    {record.diseaseName || (record.healthStatus === 'Healthy' ? 'Healthy Foliage' : 'Unknown Condition')}
                  </h4>
                  <div className="flex items-center gap-1.5 pt-0.5">
                    <span className={`text-[8px] font-sans font-bold uppercase tracking-wider px-1.5 py-0.5 ${
                    ...
                    {record.confidenceScore && (
                      <span className="text-[8px] text-neutral-400 font-mono">
                        {Math.round(record.confidenceScore * 100)}% Conf
                      </span>
                    )}
                  </div>
                </div>
